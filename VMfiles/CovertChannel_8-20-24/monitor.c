...
{   
    ...
    InitChannelsMutexes();
    http_port_index = 1;
    Init_Ports_Cache(&cache[0]);
    Init_Ports_Cache(&cache[1]);
...
int callbackHTTP(struct nfq_q_handle *qh, struct nfgenmsg *nfmsg,
    ...
    if (ph) {
        int id = ntohl(ph->packet_id), temp;
        
        pthread_mutex_lock(&mutex_callback1);
        
        if ((temp = Check_Port(srcport)) == -1) {
            printf("hjh\n");
            temp = http_port_index = 1 - http_port_index;
            Add_Port(&(cache[http_port_index]), srcport);
            printf("jhj\n");
        }
        push(cache[http_port_index].ports[temp].qport, id);
        printf("id = %d, push = %d\n", temp, srcport);
        if (tcp_header->fin) {
            cache[http_port_index].ports[cache->port_index].isFin = true;
        }
...
void Init_Ports_Cache(Ports_Cache* cache) {
    //for (int i = 0; i < 5; i++) cache->ports[i] = NULL;
    cache->port_index = 0;
}

void Add_Port(Ports_Cache* cache, int srcport) {
    Port_Queue q;
    q.qport = malloc(sizeof(queue));
    if (q.qport != NULL) {
        q.qport->first = q.qport->last = NULL;
        q.qport->counter = 0;
    }
    q.isFin = false;
    q.srcport = srcport;

    cache->ports[cache->port_index] = q;
    cache->port_index = (cache->port_index + 1) % 5;
}

int Check_Port(int port) {
    for (int i = 0; i < 5; i++) {
        for (int j = 0; j < 2; j++) {
            if (cache[j].ports[i].srcport == port)
                return j;
        }
    }
    return -1;
